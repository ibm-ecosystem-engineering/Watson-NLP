...
def get_sentiment(text):
    return grpcClient.call_sentiment_document_workflow(text)
...
def sentiment_analysis_callback(n_clicks, value):
    ...
    response = get_sentiment(value)
    sentiment_output_python = response.sentiment_mentions
    sentence_sentiment = [item.score for item in sentiment_output_python]
    ...
    sentence_sentiment = [(sm.span.text, sm.label, sm.score) for sm in sentiment_output_python]
    sentence_list = []
    label_list = []
    score_list = []
    ...
    return (response.score, response.label), fig_sentiment, df_sentiment_output.to_dict('records')
