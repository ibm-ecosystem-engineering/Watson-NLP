...
class GrpcClient:
    ...
    def call_sentiment_model(self, inputText):
        ...
        SENTIMENT_DOCUMENT_CNN_WORKFLOW_MODEL = os.getenv(
            "SENTIMENT_DOCUMENT_CNN_WORKFLOW_MODEL",
            default="sentiment_aggregated-cnn-workflow_lang_en_stock",
        )
        response = self.stub.SentimentPredict(
            request, metadata=[("mm-model-id", SENTIMENT_DOCUMENT_CNN_WORKFLOW_MODEL)]
        )
        return response
    ...
    def call_emotion_model(self, inputText):
        ...
        EMOTION_CLASSIFICATION_STOCK_MODEL = os.getenv(
            "EMOTION_CLASSIFICATION_STOCK_MODEL",
            default="classification_ensemble-workflow_lang_en_tone-stock",
        )
        response = self.stub.ClassificationPredict(
            request, metadata=[("mm-model-id", EMOTION_CLASSIFICATION_STOCK_MODEL)]
        )
        return response
